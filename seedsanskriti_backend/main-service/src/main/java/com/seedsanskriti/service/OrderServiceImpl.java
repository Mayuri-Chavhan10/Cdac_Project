...
public class OrderServiceImpl implements OrderService {
	...
	public String cancelOrder(Long orderId,
	    ...
	    // so downstream reporting/reconciliation stays consistent. Payment
	    // data now lives in the Payment Service, so this is a best-effort
	    // remote call - it must not block order cancellation from succeeding
	    // if the Payment Service is temporarily unreachable.
	    try {
	        paymentServiceClient.markRefunded(order.getId());
	    } catch (Exception ex) {
	        log.warn("Order {} was cancelled but marking its payment as refunded failed - "
	                + "manual reconciliation may be required", order.getId(), ex);
	    }
